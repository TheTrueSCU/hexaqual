...
def test_generate_main_dispatches(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "hexaqual.commands.import_linter.get_repo_root",
            return_value=tmp_path,
        ),
        patch(
            "hexaqual.commands.import_linter.create_governance_bus",
...
def test_run_main_dispatches_and_presents(tmp_path: Path) -> None:
    ...
    with (
        patch("hexaqual.commands.import_linter.ensure_tool_installed"),
        patch(
            "hexaqual.commands.import_linter.get_repo_root",
            return_value=tmp_path,
        ),
        patch(
            "hexaqual.commands.import_linter.get_packages_directory",
            return_value=tmp_path,
        ),
        patch(
            "hexaqual.commands.import_linter.get_package_directories",
        ...
        patch(
            "hexaqual.commands.import_linter.create_dependency_presenter",
