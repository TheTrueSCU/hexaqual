...
def test_detect_git_targets(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "hexaqual.commands.sanity_check.get_package_directory",
            return_value=tmp_path,
        ),
        patch(
            "hexaqual.commands.sanity_check.get_example_directory",
            return_value=tmp_path,
        ),
...
def test_parser_and_main(tmp_path: Path) -> None:
    ...
    assert parsed.skip_deptry is True
    assert parsed.skip_typecheck is True
    ...
    with (
        ...
        patch(
            "hexaqual.commands.sanity_check.get_repo_root",
            return_value=tmp_path,
        ),
        patch(
            "hexaqual.commands.sanity_check.resolve_targets",
            return_value=[SanityTarget("cqrs", "package", tmp_path, (), ())],
        ),
        patch(
            "hexaqual.commands.sanity_check.run_sanity_check",
            return_value=0,
        ) as mock_run,
        ...
        assert mock_run.call_args[1]["skip_deptry"] is True
