...
from hexaqual.commands.complexipy import main, run_complexipy
...
def test_run_complexipy_fails_on_process_error():
    """Verify run_complexipy propagates non-zero exit code when complexipy fails."""
    with patch("subprocess.run") as mock_run:
        mock_proc = MagicMock()
        mock_proc.returncode = 2
        mock_proc.stdout = ""
        mock_proc.stderr = "Error: invalid argument"
        mock_run.return_value = mock_proc
