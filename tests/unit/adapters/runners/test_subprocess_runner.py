...
from hexaqual.domain.governance import CheckStatus, SanityTarget
...
def test_run_all_statements_and_parity(tmp_path: Path):
    ...
    with (
        patch(
            "hexaqual.adapters.runners.subprocess_runner.check_file_all",
            return_value=["__all__ out of order"],
        ),
        patch("hexaqual.adapters.runners.subprocess_runner.fix_file_all"),
    ):
    ...
    with patch(
        "hexaqual.adapters.runners.subprocess_runner.check_package_parity",
        return_value=[],
    ):
