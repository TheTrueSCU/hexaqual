...
def test_audit_extras_parity(tmp_path: Path):
    ...
    with (
        patch(
            "hexaqual.adapters.runners.dependency_runner.check_extras",
            return_value=[],
        ),
        patch(
            "hexaqual.adapters.runners.dependency_runner.get_package_directories",
...
def test_generate_extras_diagram(tmp_path: Path):
    ...
    adapter = SubprocessDependencyAuditorAdapter()
    with patch(
        "hexaqual.adapters.runners.dependency_runner.make_mermaid",
...
def test_generate_config_and_tool_avail(tmp_path: Path):
    ...
    with patch(
        "hexaqual.adapters.runners.dependency_runner.update_pyproject_toml",
        return_value=True,
    ):
    ...
    with patch(
        "hexaqual.adapters.runners.dependency_runner.check_tool_avail",
        return_value=(True, ""),
    ):
